...
export default class TriggersPanel extends React.Component {
  ...
  fetchGroups() {
    return store.dispatch({
      type: 'FETCH_GROUPS',
      promise: Promise.all([
        api.client.triggerTypes.list()
          .catch((err) => {
            notification.error('Unable to retrieve trigger types.', { err });
            throw err;
          }),
        api.client.index.request({ method: 'get', path: '/sensortypes' })
          .then(res => res.data)
          .catch((err) => {
        ...
        for (const sensor of sensors) {
          for (const ref of sensor.trigger_types) {
            const trigger = triggers.find(item => item.ref === ref);
            if (trigger) {
      ...
      .then(() => {
        ...
        if (id && groups && !groups.some(({ triggers }) => triggers.some(({ ref }) => ref === id))) {
              ...
              return (
                <FlexTableWrapper key={pack} uid={pack} title={pack} icon={icon}>
                  { triggers.map((trigger) => (
                    <TriggersFlexCard
                      key={trigger.ref} trigger={trigger}
